  ...
  useEffect(() => {
    ...
    const unsub = onValue(contactsRef, async (snap) => {
      ...
      const enriched = await Promise.all(
        entries.map(async (contact) => {
          const [userSnap, lastSnap] = await Promise.all([
            get(ref(db, `users/${contact.uid}`)),
            get(ref(db, `chats/${getChatId(user.uid, contact.uid)}/lastMsg`)),
          ]);
          const nextUser = userSnap.exists() ? userSnap.val() : {};
          const last = lastSnap.exists() ? lastSnap.val() : null;
          return { ...nextUser, lastMsg: last };
        }),
      );
  ...
  return (
    <div className="screen">
      ...
      <div className="conn-bar">
        ...
        <span>Koneksi ringan untuk chat dan panggilan live</span>
      </div>
      ...
      <div className="scroll-area">
        <div className="chat-row" onClick={onOpenPublic} style={{ background: 'var(--surface-low)' }}>
          ...
          <div className="chat-row-body">
            ...
            <div className="chat-row-bottom">
              <span className="chat-row-preview">Diskusi umum berbasis teks untuk semua pengguna</span>
            </div>
          </div>
        </div>

        <div className="section-label">Pesan pribadi</div>
        ...
        {loading ? (
          ...
          chats.map((contact) => <ChatRow key={contact.uid} c={contact} onOpen={() => onOpenChat(contact)} />)
        )}
      </div>
    </div>
  );
...
function ChatRow({ c, onOpen }) {
  const preview = c.lastMsg ? c.lastMsg.text : 'Mulai percakapan';
